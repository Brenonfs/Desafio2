...
describe('Test Student', () => {
  ...
  it('Error Update Student para uma turma no mesmo horário ', async () => {
    ...
    expect(secondResponse.text).toContain('Este aluno já está cadastrado em outra turma no mesmo dia e horário.');
  ...
  it('Error Update Student para a mesma turma ', async () => {
    ...
    expect(secondResponse.text).toContain('Este aluno já está cadastrado nesta turma.');
    expect(secondResponse.statusCode).toEqual(400);
  }, 10000);
  it('Error Update Student limite de vagas ', async () => {
    ...
    expect(response.text).toContain('Esta turma não foi encontrada ou já está lotada.');
...
describe('Test Teacher', () => {
  ...
  it('Error Update Teacher para uma turma no mesmo horário ', async () => {
    ...
    expect(secondResponse.text).toContain('Este professor já está cadastrado em outra turma no mesmo dia e horário.');
    expect(secondResponse.statusCode).toEqual(400);
  }, 10000);
  it('Error Update Teacher para a mesma turma', async () => {
    ...
    expect(secondResponse.text).toContain('Professor(a) já está cadastrado nesta turma.');
    expect(secondResponse.statusCode).toEqual(400);
  }, 10000);
